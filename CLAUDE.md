...
- **Cycle de vie du spare = uniquement pendant la lecture active.** `_prepareNext()` (qui calcule `_planned` — random shuffle roulé **une fois** — et **bufferise** le suivant dans `audioEl2` via `src`+`load()`, blob offline minté à l'avance) est appelé **sur l'événement `playing`** (audio confirmé en train de couler), PAS dans le chemin synchrone de `play()`/resume. `_releaseSpare()` **vide** `audioEl2` sur **pause**. ⚠️ **Pourquoi** : un 2e `<audio>` "prêt" et idle pendant la ré-acquisition de session iOS (au resume) fait router le son vers lui → l'élément actif avance **en silence**. En sortant le spare des fenêtres fragiles (resume, restore en pause, shuffle/repeat/mutations-en-pause via le garde `!paused`), le resume ré-acquiert la session avec **un seul** élément. L'état stable (les deux éléments vivants pendant la lecture) marche, lui.
- `_swapToPreloaded()` **échange les refs** `audioEl`↔`audioEl2` et joue l'élément déjà bufferisé — transition sans fetch, donc sans trou (le trou fait tomber la session iOS). Appelé par `_onAudioEnded` ET `next()`. Le nouveau spare se rebâtit ensuite sur le `playing` de l'élément promu.
- `loadAndPlay` (sauts **non séquentiels** : tap, `playFromList`, `prev`, restore) charge sur l'élément actif, online en **sync** (`trackPlayUrl`), offline awaité (blob obligatoire, iOS bypasse le SW). Il `_discardPreloaded()` le plan stale en entrée ; le spare se rebâtira sur `playing`.
...
Garde-fous si tu touches à ça : ne bâtis JAMAIS le spare dans le chemin synchrone de `play()`/resume (laisse l'event `playing` le faire) ni pendant que c'est en pause ; compare `audioEl2.src` **normalisé** (relatif vs absolu) avant de swapper ; swappe les refs **avant** de `pause()` l'ancien (sinon son event `pause` flippe `playing`) ; révoque le blob sortant **après** avoir vidé le `src`. Vérifié en preview (store instrumenté) : spare bâti sur `playing`, vidé sur pause, **absent au resume sync** (un seul élément), swap synchrone sur `ended`/`next`, ping-pong sur N pistes, wrap repeat='all', stop en fin de file. Le cas `ended`-pas-déclenché-en-background reste hors de portée (iOS).
