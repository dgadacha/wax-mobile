...
- **yt-dlp uses `--extractor-args "youtube:player_client=web,android"`** — `web` first because it exposes audio-only m4a (itag 140), `android` as fallback (faster but only returns the combined 360p mp4 itag 18). We tried `android` first earlier (~2.5× faster URL extraction) but feeding the combined mp4 into `<audio>` floods Chromium with `Unsupported pixel format: -1` errors on every track since the demuxer can't parse the video stream. Slower extraction (~1-2 s vs ~0.5 s) is worth the silent log.
