...
- **`warmOfflineCache`** (re-fetch des MP3 téléchargés vers le Cache Storage) = **bouton manuel uniquement** (Réglages → Hors-ligne → "Vérifier le cache"). ⛔ Ne le rebranche PAS en auto au boot / sur l'event online : s'il crashe l'onglet (OOM iOS), l'auto-run transforme un crash unique en **boucle de reload infinie** ("un problème récurrent est survenu"). ⛔ Et dans le worker, `cache.put(url, res)` — **jamais `res.clone()`** : cloner une Response dont l'autre branche n'est jamais lue bufferise le MP3 entier en RAM JS → OOM avec POOL workers en vol. ⛔ **Le `fetch` du warmer DOIT contourner le SW** (`?warm=1` sur l'URL — la règle CacheFirst `/audio/*.mp3$` est ancrée `$`, le query dodge) : sinon le SW **intercepte aussi** ce fetch et Workbox CacheFirst fait son propre `response.clone()` + `cache.put` → chaque titre est bufferisé **2×** (tee du SW + notre put) et écrit **2×** dans `wax-audio`. C'était le vrai leak (clone invisible côté SW) que baisser POOL 6→4→2 (0.19.12/0.19.13) n'a jamais corrigé. **Fix 0.26.2** : `?warm=1` (writer unique = la page) + **POOL=1** (strictement séquentiel, un seul MP3 bufferisé à la fois). ⛔ Ne remonte PAS POOL. Le fix OOM iOS reste invérifiable en preview Chrome (seule la logique : bypass regex, clé de cache propre, écriture unique) → device-test.

**0.27.0 — sélection par playlist + erreurs durcies.** `warmOfflineCache(onProgress, trackIds=null)` accepte un scope optionnel (`null` = toute la biblio). ⛔ **Le scope EST la meilleure parade au crash iOS** : borner un run à une playlist (~15 titres) écrit bien moins dans Cache Storage que 90+ titres d'un coup. UI : le bouton "Vérifier le cache" ouvre un **sheet de sélection** (`ViewSettings` → `warmPickerOpen`) — "Toute la bibliothèque" OU cases par **Favoris + playlists** (uniquement les scopes qui ont des titres téléchargés ; ids dédupliqués en `Set`). Erreurs séparées (fini le `catch` fourre-tout qui droppait `track.file` sur **toute** erreur) : **404 seul** droppe `file` (fichier vraiment perdu côté serveur) ; un **échec réseau** compte l'échec sans toucher `file` ; un **throw de `cache.put`** (quota iOS) → `result.quotaHit=true` + stop **gracieux** (pas de drop) au lieu de pousser jusqu'au kill de l'onglet. Vérifié en preview : sheet peuplé (Favoris + playlist), toggle réactif, dédup (union `Set`), fetch en `?warm=1`, MP3 réel rangé sous la clé **propre** (playback le retrouve via `cache.match` ignoreSearch), 404 gérés sans crash, résumé "N en cache · +N pré-téléchargés · N échecs".
...
> 🔑 **Pattern des jobs IA (génération + tagging) — IMPORTANT.** ⛔ **Ne JAMAIS streamer du SSE sur la réponse d'un POST** : en prod, les reverse-proxies (Cloudflare & co) **bufferisent les réponses POST** → les frames n'arrivent jamais au client (symptôme : pas de barre de progression + « Génération interrompue »). Tenté en 0.20.2/0.21.0, cassé en prod, **reverté en 0.21.1**. Le pattern qui marche (= celui des téléchargements) : **`POST /api/ai/*` démarre un job en arrière-plan et renvoie `{jobId}` en JSON** ; le client observe via **`GET /api/ai/jobs/:id` (EventSource, GET text/event-stream passe les proxies)**. Le travail tourne côté serveur **indépendamment de la connexion** → une reconnexion EventSource ré-attache (le GET renvoie l'état courant `total`+`progress` à la connexion, puis le live), **ne RELANCE jamais** le job. Registre `aiJobs` (Map), helpers `aiEmit`/`aiFinishOk`/`aiFinishErr`/`aiCloseJob` (GC du job +60s). Heartbeat `: hb` toutes les 15s sur le GET (anti-idle pendant l'appel Claude). Client : `runAiJob(startPath, body, onEvent)` dans `lib/api.js` (POST start → EventSource → resolve sur `done` / reject sur `error`).

- **Serveur** (`server.cjs`, `POST /api/ai/playlist` → `runAiPlaylistJob`) : `getAnthropic()` = client **lazy** (`require('@anthropic-ai/sdk')` + `new Anthropic()` seulement si `ANTHROPIC_API_KEY` présente, sinon **null** → le POST renvoie **503**, le serveur boote toujours sans clé). Modèle `claude-haiku-4-5` (1$/5$ par M tokens), **structured outputs** (`AI_PLAYLIST_SCHEMA`) → `{name, tracks:[{title,artist}]}`, **50 titres** (`max_tokens: 3000`, cap 60). ⚠️ Haiku **ne supporte pas** `effort` ni le thinking. Chaque suggestion → `aiResolveTrack()` (`ytsearch1` via le sémaphore `runYtDlp`, `null` si miss → `Promise.all` ne casse jamais), pliée en biblio `liked:false` (de-dup `ytId`), puis playlist (`aiPrompt` en méta). Events : `total {total,name}` → `progress {done,total,found}` → `done {playlist,resolved}` | `error`. **La clé ne quitte JAMAIS le backend.**
- **Front** : overlay singleton `AiPlaylistSheet.vue` (monté dans `App.vue`), piloté par `view.aiOpen` / `view.openAi()` / `view.closeAi()`. **Toggle 2 modes** : *Découvrir* (défaut → `runAiJob('/api/ai/playlist')`, barre « N/50 titres trouvés », titres neufs résolus sur YouTube) ou *Ma bibliothèque* (→ `playlists.generateFromLibrary(prompt)`, **POST JSON simple** `/api/ai/playlist/from-library` : Claude pioche des index dans la biblio numérotée, aucune résolution YouTube, rapide). Au succès : refetch → toast → `view.switchTo('playlist', id)`. Deux points d'entrée : carte "Playlist IA" de la grille **Parcourir** (Search) + le **"+"** de la Bibliothèque.
- **Déploiement** : `ANTHROPIC_API_KEY` via secret K8s `wax-ai` (clé `anthropic-api-key`), injectée dans le pod en `optional: true` (cf. `k8s/deployment.yaml`). Le secret est **créé par la CI** (stage `deploy`) depuis la variable CI/CD GitLab `ANTHROPIC_API_KEY` — pas de kubectl manuel (cf. section Secrets K8s). `@anthropic-ai/sdk` est en `dependencies` mais **uniquement `require` par `server.cjs`** → jamais bundlé côté front (Vite n'importe rien depuis `src/`).

### Tags d'ambiance / énergie (mood tagging)
...
- **Renommer avec l'IA** (`POST /api/ai/playlists/:id/rename`) : un appel Claude nomme la playlist d'après sa tracklist (2-4 mots) — pratique pour les **mix auto-nommés "Mix · …"**. `playlists.renameWithAI(id)`, item "Renommer avec l'IA" dans le même menu ⋮. ⚠️ Un mix **sauvegardé** EST une vraie playlist (`mix.save` le pousse dans `playlists`) → il hérite donc de reorder + rename automatiquement.
...
### Radio infinie — la file se prolonge toute seule

5e feature IA : quand on approche de la fin de la file, l'IA ajoute des titres dans la même veine pour ne jamais s'arrêter (façon "radio" Spotify).
- **Serveur** (`POST /api/ai/radio` → `runAiRadioJob`, **pattern de job**) : reçoit `{seeds:[string]}` (les derniers titres écoutés en « Artiste - Titre »), Haiku recommande 12 titres proches (réutilise `AI_PLAYLIST_SCHEMA`, `name` ignoré), résolus via `aiResolveTrack`, de-dup par `id`. Renvoie `{tracks:[{id,title,uploader,duration,thumbnail}]}` — **éphémères** (streams, pliés en biblio seulement si écoutés >30s). 400 si pas de seeds, 503 sans clé.
- **Front** (`stores/player.js`) : état `radio` (toggle de session, **non persisté** — la file contient des streams donc `savePlayerState` la skip) + `_radioBusy` (garde anti-réentrance). `toggleRadio()` (bouton **Radio** dans la rangée extras du `MobilePlayer`, à gauche des paroles). `_maybeExtendRadio()` est appelé à **chaque avance** (fin de `loadAndPlay` + fin de `_swapToPreloaded`) : si `radio` actif, pas déjà busy, `repeat==='off'`, et ≤2 titres restants après le courant → `_extendRadio()`. Celui-ci construit les seeds via `parseTrackTitle`, appelle `runAiJob('/api/ai/radio')`, enregistre chaque reco dans le store `streams` (`stream-<ytId>`) et **append** à la file (de-dup contre la file courante), puis `_prepareNext()`. ⛔ **Ne touche PAS** aux internals audio/swap (cf. garde-fous iOS) — la radio ne fait qu'ajouter des ids à `queue` et laisse la machinerie d'enchaînement faire le reste. Échec réseau → `console.warn` + `_radioBusy` réarmé, le toggle reste actif.

### Nettoyage des titres (IA)

6e feature IA : extrait un **artiste + titre propres** des titres YouTube crades ("chase atlantic - swim [ slowed + reverb ] (lyrics)" → artiste "Chase Atlantic", titre "Swim").
- **Serveur** (`POST /api/ai/clean-titles` → `runAiCleanJob`, **pattern de job**, batch de 25) : tague les titres **sans `titleClean`** (ou tous si `{force:true}`) via Haiku (structured output `AI_CLEAN_SCHEMA` = `{items:[{i,artist,title}]}`). Écrit `track.artistClean`/`titleClean`/`cleanedAt` dans `library.json` (**sans toucher `title`/`uploader`** qui servent à re-chercher sur YouTube), **sauve après chaque batch** (resumable, batch en échec skippé). Events : `total` → `progress` (par batch) → `done {cleaned,total}`. **Pas de yt-dlp** (pure classification).
- **Front** : déclencheur dans **Réglages → Bibliothèque → "Nettoyer les titres (IA)"** (`lib.cleanTitles(onProgress)` via `runAiJob`, barre de progression façon "Analyser ma bibliothèque", refetch en fin). Consommation : helpers `displayTitle(track)` / `displayArtist(track)` dans `lib/format.js` (préfèrent les champs clean, **fallback sur le brut → aucune régression** sur les titres non nettoyés), utilisés par `MobileTrackCell` + `MobilePlayer`. `parseTrackTitle` préfère aussi `titleClean`/`artistClean` quand présents.
