...
- **`server.js`** (~700 lines) — Express. Single-file, no framework beyond Express. Spawns `yt-dlp` for: search (`ytsearch10:`), playlist enumeration (`--flat-playlist`), single-track stream URL extraction (`-g`), MP3 download (`-x --audio-format mp3`), Mix enumeration (`RD<videoId>` playlist), preview clips (`--download-sections`).
  ...
  - **Album metadata pipeline** (`/api/album-lookup?artist=&title=` + `/api/album-cover/:mbidReleaseGroup`): yt-dlp doesn't expose any `album` field for YouTube videos (verified across regular videos, Topic auto-channels, and `music.youtube.com` URLs — every variant returns NA), so we resolve album info via **MusicBrainz** instead. Query: `artistname:"X" AND recording:"Y"` (`artistname` not `artist` — multi-credit recordings like "Daft Punk feat. Pharrell" don't match strict `artist:`). We run the query twice in parallel — once unfiltered, once with `AND NOT secondarytype:Live AND NOT secondarytype:Compilation` — and pick whichever pool yields the higher-scored release-group, because bootleg-flooded artists (Radiohead, Phish, Grateful Dead) drown the unfiltered results in live recordings, while normal queries work fine without the filter. Scoring: primary-type Album = 100 base, EP = 60, Single = 50, Soundtrack = 40; -60 for `Compilation` secondary-type, -35 for `Live`/`Remix`, -30 for `Demo`, -25 for `Mixtape/Street`. Title-based deboost regex (`TITLE_DEMOTE`) catches "Deluxe / Collectors / Anniversary / Remastered / Special Edition / OST / Soundtrack" patterns at -40, and `TITLE_DATE_PREFIX` (`/^\d{4}\D\d{2}\D\d{2}/`, handles unicode hyphens) at -80 to kill live bootleg titles like "2003-06-04: Electric Lady Studios". Hits-bonus = +3 per recording referencing the release-group (capped at +20) — canonical originals are referenced by many recordings. Rejects pools where best score < 30. Rate-limited to 1 req/sec with mandatory User-Agent (`Wax/<version> (https://github.com/dgadacha/wax)`); 503 from MB → null (treated as "no match for now"). Disk cache: `library/albums/<sha1(normalizedArtist|normalizedTitle)>.json`, miss sentinel `.miss` re-tried after 7 days. **Cover art** comes from Cover Art Archive — `https://coverartarchive.org/release-group/<rgId>/front-500` proxied through `/api/album-cover/:mbid` (follows redirects up to 4 hops, caches to `library/album-covers/<rgId>.jpg`). Accuracy on a 16-track test set: 13/16 = 81 % — the misses are still albums by the right artist (e.g. DAMN. Collectors Edition vs DAMN.). **Auto-backfill** is wired into `POST /api/library/add`: every newly-added track gets a fire-and-forget MB lookup that writes `album` / `albumReleaseGroupId` / `albumReleaseDate` back into library.json on success. **Bulk backfill** for existing libraries: `POST /api/library/backfill-albums` (idempotent, returns running state if already in progress) + `GET /api/library/backfill-albums` (status). Driven by Settings → General → "Backfill albums" with a live progress bar polling once every 1.5 s. `parseTrackTitle` mirrored server-side to clean YouTube cruft before querying MB.
...
- `ViewAlbums.vue` — index page surfaced by the sidebar "Albums" entry. Renders `lib.albums` as a square-cover grid (`grid-template-columns: repeat(auto-fill, minmax(180px, 1fr))`); each card pulls its cover from `/api/album-cover/<rgId>` (Cover Art Archive proxy) and falls back to `gradientFromString(album.name)` when the release-group has no art or 404s. Click → `view.switchTo('album', key)`.
...
- `SettingsBody.vue` — Settings modal content, organized as **3 tabs** (`activeTab` ref, default `'general'`): **General** (Crossfade toggle + duration slider, Language picker — `prefs.setLocale(id)` over `SUPPORTED_LOCALES`, Backup — Export / Import via `@/lib/backup` with an inline progress bar, Library cleanup — orphan count + "Clean" → `lib.purgeOrphans`, **Albums backfill** — POSTs `/api/library/backfill-albums` then polls every 1.5 s, shows `done/total` + a progress bar; on completion `lib.fetch()` re-loads so the new `album` fields hit the artist + albums views immediately, **Reset** danger zone — `wipeAllData()` with a strongly-worded confirm modal showing the exact counts to be deleted), **Theme** (theme picker — two sub-grids, dark and light, `prefs.setTheme(id)`), **Equalizer** (3-band ±12 dB sliders → `setEq` from `useVisualizer` + persisted in `prefs.eq`). All labels in the modal use `t()` so the tabs and section content re-render in the chosen language. The import and reset flows both reload the page on success so every store re-fetches against the freshly written state.
