...
- **yt-dlp uses `--extractor-args "youtube:player_client=ios,web,android"`** plus `-f 'bestaudio[ext=m4a]/bestaudio'` (no `/best` fallback). Client priority: `ios` first (fast, no SABR/sig dance, exposes audio-only m4a itag 140), `web` second (slower but reliable, also audio-only), `android` last resort (only returns the combined 360p mp4 itag 18 which floods Chromium with `Unsupported pixel format: -1`). Dropping the `/best` fallback in `-f` means we'd rather error out than serve a video stream into `<audio>` — silent logs over silent video. The 2.5× faster android-first variant we tried earlier (v1.21–1.24.1) was the source of the pixel-format spam. There's also a 25 s safety timeout on the spawn so a wedged yt-dlp process can't hold a semaphore slot forever.
