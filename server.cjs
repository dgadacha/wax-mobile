...
// ----------------------------------------------------------------
// Spotify import — paste a public playlist URL → search each track on
// YouTube → add to Wax library + optionally create a Wax playlist.
//
// Requires WAX_SPOTIFY_CLIENT_ID + WAX_SPOTIFY_CLIENT_SECRET (free
// Spotify Developer app, Client Credentials flow — no user OAuth).
// ----------------------------------------------------------------
const SPOTIFY_CLIENT_ID = process.env.WAX_SPOTIFY_CLIENT_ID || '';
const SPOTIFY_CLIENT_SECRET = process.env.WAX_SPOTIFY_CLIENT_SECRET || '';

let _spotifyToken = null;
let _spotifyTokenExpiry = 0;

function spotifyEnabled() { return !!(SPOTIFY_CLIENT_ID && SPOTIFY_CLIENT_SECRET); }

function getSpotifyToken() {
  if (_spotifyToken && Date.now() < _spotifyTokenExpiry) return Promise.resolve(_spotifyToken);
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      tokenRes.on('end', () => {
        try {
          const json = JSON.parse(data);
          if (!json.access_token) return reject(new Error('Spotify auth failed: ' + (json.error_description || json.error || 'unknown')));
...
function fetchSpotifyJson(apiPath, token) {
  return new Promise((resolve, reject) => {
    https.get({
      ...
      if (spotRes.statusCode === 404) { spotRes.resume(); return reject(new Error('Playlist introuvable (privée ou inexistante)')); }
      if (spotRes.statusCode !== 200) { spotRes.resume(); return reject(new Error(`Spotify HTTP ${spotRes.statusCode}`)); }
      let data = '';
      spotRes.on('data', (c) => { data += c; });
      spotRes.on('end', () => {
        try { resolve(JSON.parse(data)); } catch (e) { reject(e); }
      });
    }).on('error', reject);
...
async function fetchAllSpotifyTracks(playlistId, token) {
  ...
  while (true) {
    const data = await fetchSpotifyJson(
      `/v1/playlists/${encodeURIComponent(playlistId)}/tracks?limit=${limit}&offset=${offset}&fields=items(track(id,name,artists,duration_ms)),next`,
      token,
    );
    for (const item of (data.items || [])) {
      ...
      tracks.push({ spotifyId: tr.id || '', name: tr.name, artist, duration_ms: tr.duration_ms || 0 });
    }
    if (!data.next) break;
    offset += limit;
...
function extractSpotifyPlaylistId(input) {
  ...
  if (m) return m[1];
  if (/^[A-Za-z0-9]{22}$/.test(String(input || '').trim())) return String(input).trim();
  return null;
}

function ytSearch1(query) {
  return runYtDlp(() => new Promise((resolve) => {
    ...
    const timer = setTimeout(() => { try { ytdlp.kill('SIGKILL'); } catch {} resolve(null); }, 20000);
    ytdlp.on('close', (code) => {
      clearTimeout(timer);
      if (code !== 0 || !out.trim()) return resolve(null);
      const [id, title, uploader, duration] = out.trim().split('\n')[0].split('|||');
...
app.get('/api/spotify/status', (_req, res) => {
  res.json({ configured: spotifyEnabled(), clientId: SPOTIFY_CLIENT_ID });
});

// Generic import: frontend sends {name, artist}[] (used for liked tracks + OAuth playlists)
app.post('/api/spotify/import-tracks', (req, res) => {
  const { tracks: rawTracks, createPlaylist, playlistName } = req.body || {};
  if (!Array.isArray(rawTracks) || rawTracks.length === 0) {
    return res.status(400).json({ error: 'Aucun titre fourni' });
  }
  const tracks = rawTracks.slice(0, 1000).filter((t) => t && typeof t.name === 'string');

  const jobId = crypto.randomBytes(8).toString('hex');
  const job = {
    id: jobId,
    type: 'spotify-import-tracks',
    progress: 0,
    phase: 'searching',
    status: 'pending',
    track: null,
  ...
  setImmediate(async () => {
    try {
      ...
      broadcast(job, { type: 'progress', progress: 0, phase: 'searching', done: 0, total });
      ...
      for (let i = 0; i < tracks.length; i++) {
        ...
        const sp = tracks[i];
        const query = sp.artist ? `${sp.artist} ${sp.name}` : sp.name;
        ...
        let result = null;
        try { result = await ytSearch1(query); } catch {}

        if (!result) { skipped++; continue; }

        const lib = getLibrary(profileId);
        const existing = lib.find((t) => t.ytId === result.id);
        let trackId;
        if (existing) {
          trackId = existing.id;
        } else {
      ...
      if (createPlaylist && importedTrackIds.length > 0 && playlistName) {
        const pls = getPlaylists(profileId);
        const newId = crypto.randomBytes(8).toString('hex');
        pls.push({
          id: newId,
          name: String(playlistName).slice(0, 100),
      ...
      job.status = 'ready';
      job.progress = 100;
      broadcast(job, { type: 'ready', imported, skipped, total });
    } catch (err) {
      job.status = 'error';
      job.error = err.message;
      broadcast(job, { type: 'error', error: err.message });
...
app.post('/api/spotify/import', (req, res) => {
  if (!spotifyEnabled()) return res.status(503).json({ error: 'Spotify non configuré. Ajoute WAX_SPOTIFY_CLIENT_ID et WAX_SPOTIFY_CLIENT_SECRET.' });
  const { url: rawUrl, createPlaylist } = req.body || {};
  const playlistId = extractSpotifyPlaylistId(String(rawUrl || ''));
  if (!playlistId) return res.status(400).json({ error: 'URL Spotify invalide' });
  ...
  setImmediate(async () => {
    try {
      ...
      let playlistName = 'Spotify import';
      try {
        const plInfo = await fetchSpotifyJson(
          `/v1/playlists/${encodeURIComponent(playlistId)}?fields=name`,
          token,
        );
        if (plInfo.name) playlistName = plInfo.name;
      } catch {}

      const spotifyTracks = await fetchAllSpotifyTracks(playlistId, token);
      ...
      if (createPlaylist && importedTrackIds.length > 0) {
        const pls = getPlaylists(profileId);
        newPlaylistId = crypto.randomBytes(8).toString('hex');
        pls.push({
          id: newPlaylistId,
          name: playlistName,
          trackIds: importedTrackIds,
      ...
      job.status = 'ready';
      job.progress = 100;
      broadcast(job, { type: 'ready', imported, skipped, total, playlistId: newPlaylistId });
    } catch (err) {
      job.status = 'error';
      job.error = err.message;
      broadcast(job, { type: 'error', error: err.message });
